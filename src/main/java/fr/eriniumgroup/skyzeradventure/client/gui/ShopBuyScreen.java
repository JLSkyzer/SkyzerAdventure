...
import fr.eriniumgroup.skyzeradventure.network.ShopBuyButtonMessage;
...
public class ShopBuyScreen extends AbstractContainerScreen<ShopBuyMenu> {
	private final static HashMap<String, Object> guistate = ShopBuyMenu.guistate;
	...
	public ShopBuyScreen(ShopBuyMenu container, Inventory inventory, Component text) {
	...
	private static final ResourceLocation texture = new ResourceLocation("skyzeradventure:textures/screens/shop_buy.png");
	...
	protected void renderLabels(PoseStack poseStack, int mouseX, int mouseY) {
		this.font.draw(poseStack, new TranslatableComponent("gui.skyzeradventure.shop_buy.label_shop_gui"), 60, 8, -1);
		this.font.draw(poseStack,

				ReturnMoneyTextProcedure.execute(entity), 107, 9, -12829636);
	}
	...
	public void init() {
		...
		search = new EditBox(this.font, this.leftPos + 225, this.topPos + 3, 144, 20, new TranslatableComponent("gui.skyzeradventure.shop_buy.search")) {
			{
				setSuggestion(new TranslatableComponent("gui.skyzeradventure.shop_buy.search").getString());
				setValue(playercap.buysearch);
			}
		...
		button_return = new Button(this.leftPos + 3, this.topPos + 3, 56, 20, new TranslatableComponent("gui.skyzeradventure.shop_buy.button_return"), e -> {
			if (true) {
				textstate.put("textin:search", search.getValue());
				SkyzeradventureMod.PACKET_HANDLER.sendToServer(new ShopBuyButtonMessage(0, x, y, z, textstate));
				ShopBuyButtonMessage.handleButtonAction(entity, 0, x, y, z, textstate);
			}
		});
		guistate.put("button:button_return", button_return);
		this.addRenderableWidget(button_return);
		button_search = new Button(this.leftPos + 369, this.topPos + 3, 56, 20, new TranslatableComponent("gui.skyzeradventure.shop_buy.button_search"), e -> {
			if (true) {
				textstate.put("textin:search", search.getValue());
				SkyzeradventureMod.PACKET_HANDLER.sendToServer(new ShopBuyButtonMessage(1, x, y, z, textstate));
				ShopBuyButtonMessage.handleButtonAction(entity, 1, x, y, z, textstate);
			}
		});
